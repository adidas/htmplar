...
exports.slugify = exports.setMedium = exports.getBaseStyles = exports.createID = void 0;
// utils
...
const createID = component => `htmplar-${Buffer.from(JSON.stringify(component).split('').sort(() => SORTING_THRESHOLD - Math.random()).join('')).toString('base64').substring(0, ID_LENGTH)}`;
...
const slugify = str => {
  ...
  _str = _str.replace(/[^a-z0-9 -]/g, '').replace(/\s+/g, '-').replace(/-+/g, '-');
  return _str;
};
